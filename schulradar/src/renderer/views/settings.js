  ...
  root.append(
    h(
      ...
      section(
        'Erinnerungen',
        'bell',
        row('Erinnerungen anzeigen', 'Windows-Benachrichtigungen für Abgaben und Tests.', toggle(r.enabled, (v) => setR({ enabled: v }), 'Erinnerungen')),
      ...
      section(
        ...
        row(
          'Automatisch abrufen',
          'Wie oft Schulradar im Hintergrund nach neuen Aufgaben sucht.',
      ...
      section(
        ...
        row(
          'Design',
          '',
          select(s.theme, [['system', 'wie Windows'], ['light', 'Hell'], ['dark', 'Dunkel']], (v) => set({ theme: v }))
        ),
        row('Mit Windows starten', 'Schulradar startet automatisch und erinnert dich auch ohne geöffnetes Fenster.', toggle(s.autostart, (v) => set({ autostart: v }), 'Autostart')),
        row('Beim Windows-Start nur im Infobereich', 'Das Fenster öffnet sich erst, wenn du auf das Symbol klickst.', toggle(s.startHidden, (v) => set({ startHidden: v }), 'Versteckt starten')),
        row('Schließen = im Hintergrund weiterlaufen', 'Das ✕ versteckt das Fenster nur. Beenden über das Symbol im Infobereich.', toggle(s.closeToTray, (v) => set({ closeToTray: v }), 'Im Hintergrund')),
      ...
      section(
        'Daten & Hilfe',
        'folder',
        row('Datenordner', 'Hier liegen Aufgaben, Häkchen und Einstellungen (JSON-Datei).', h('button', { class: 'btn', onclick: () => ctx.api.openDataFolder() }, icon('folder'), 'Öffnen')),
        row(
        ...
        row('Diagnose speichern', 'Protokoll aller Plattformen für die Fehlersuche (ohne Passwörter).', h('button', { class: 'btn', onclick: () => ctx.actions.exportDiagnostics(null) }, icon('download'), 'Speichern')),
      ...
      h(
        ...
        ctx.state.secureStorage ? 'Zugangsdaten werden mit Windows verschlüsselt gespeichert.' : 'Hinweis: Auf diesem System ist keine sichere Verschlüsselung verfügbar.'
      )
    )
  );
}
